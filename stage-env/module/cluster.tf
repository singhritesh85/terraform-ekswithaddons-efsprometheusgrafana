...
resource "null_resource" "kubectl" {
    provisioner "local-exec" {
        command = "aws eks --region ${data.aws_region.reg.name} update-kubeconfig --name ${aws_eks_cluster.eksdemo.name}"
    }
...
resource "aws_iam_policy" "eks_efs_csi_driver_policy" {
  name        = "EKS_EFS_CSI_Driver_Policy-${var.env}"
  description = "Policy for efs csi driver"
  policy = file("iam-policy-for-eks-efs.json")
...
resource "aws_iam_role_policy_attachment" "amazon_efs_csi_driver_kms" {
   role       = aws_iam_role.eks_efs_csi_driver.name
...
resource "aws_efs_file_system" "monitoring_efs" {
  creation_token = "monitoring-efs-${var.env}"
  encrypted      = true
  tags = {
    Name = "monitoring-efs-${var.env}"
    Environment = var.env        ##"Dev"
    Owner       = "Ops"
    Billing     = "MyProject"
  }
...
resource "aws_efs_mount_target" "eks_efs_1" {
  file_system_id = aws_efs_file_system.monitoring_efs.id
  subnet_id      = aws_subnet.private_subnet[0].id
  security_groups = [aws_security_group.efs_eks_sg.id]
}

resource "aws_efs_mount_target" "eks_efs_2" {
  file_system_id = aws_efs_file_system.monitoring_efs.id
  subnet_id      = aws_subnet.private_subnet[1].id
  security_groups = [aws_security_group.efs_eks_sg.id]
}

resource "aws_efs_mount_target" "eks_efs_3" {
  file_system_id = aws_efs_file_system.monitoring_efs.id
