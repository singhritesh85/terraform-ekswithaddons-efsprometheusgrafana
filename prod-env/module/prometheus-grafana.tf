...
resource "kubernetes_namespace" "monitor_namespace_prod" {
...
resource "kubernetes_storage_class" "eks_sc" {
  ...
  depends_on = [aws_efs_mount_target.eks_efs_1, aws_efs_mount_target.eks_efs_2, aws_efs_mount_target.eks_efs_3, kubernetes_namespace.monitor_namespace_prod]
...
resource "helm_release" "prometheus_prod" {
  name       = "prometheus"
  chart      = "prometheus"
...
resource "helm_release" "grafana" {
  ...
  depends_on = [helm_release.prometheus_prod]
}
